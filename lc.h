...
#define COUT(x) std::cout << ANSI_BLUE_BG << "[" #x "]: " << ANSI_CLEAR << toString(x) << std::endl
    ...
    do {                                                                                                              \
        if (expr) {                                                                                                   \
            ...
            std::cerr << ANSI_RED_BG << "[FAILED] " << ANSI_CLEAR << "[" + std::to_string(gTotalTest.load()) + "] "   \
                      << ANSI_BLUE_BG << #expr " => istrue? : " << ANSI_YELLOW_BG << COUT_POSITION << ANSI_CLEAR      \
                      << std::endl;                                                                                   \
...
public:
    ...
    Image(const vector<vector<int>> &data, string filename = "lc_ppm_demo",
          std::function<color(int)> mapper = Colors::map_int_to_color, string &&folder = "pic")
    ...
    Image(const vector<int> &data, string filename = "lc_ppm_demo",
          std::function<color(int)> mapper = Colors::map_int_to_color, int height = 100, string &&folder = "pic")
